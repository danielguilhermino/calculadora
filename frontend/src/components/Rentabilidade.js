...
const Rentabilidade = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError('Erro ao calcular rentabilidade. Verifique os parâmetros e tente novamente.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Grid container spacing={3}>
            {/* O formulário de cálculo continua o mesmo */}
            <Grid item xs={12}>
                <Card>
                    <CardContent>
            ...
            {/* A exibição de resultados agora é dividida */}
            {error && <Grid item xs={12}><Typography color="error">{error}</Typography></Grid>}
            {result && (
                <>
                    {/* Card de Rentabilidade Acumulada */}
                    <Grid item xs={12}>
                        <Card>
                            <CardContent>
                                ...
                                <TableBody>
                                    {result.rentabilidades_mensais.map((item) => (
                                        <TableRow key={`${item.ano}-${item.mes}`}>
                                            <TableCell>{item.ano}</TableCell>
                                            <TableCell>{item.mes.toString().padStart(2, '0')}</TableCell>
                                            <TableCell align="right">{formatPercent(item.rentabilidade)}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
